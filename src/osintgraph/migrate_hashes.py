    ...
    while True:
            ...
            for data_type in ['followers', 'followees', 'posts']:
                ...
                if hash_str:
                    try:
                        ...
                        end_cursor = hash_data.get('node', {}).get('remaining_data', {}).get('page_info', {}).get('end_cursor')
                        count = hash_data.get('node', {}).get('total_index')
                        if end_cursor is not None and count is not None:
                            new_cursor_data = json.dumps({'end_cursor': end_cursor, 'count': count})
                            update_payload[f'shared_{data_type}_cursor'] = new_cursor_data
        ...
        with neo4j_manager.get_session() as session:
            ...
            update_query = """
                ...
                SET p._shared_followers_cursor = coalesce(update.shared_followers_cursor, p._shared_followers_cursor),
                    p._shared_followees_cursor = coalesce(update.shared_followees_cursor, p._shared_followees_cursor),
                    p._shared_posts_cursor = coalesce(update.shared_posts_cursor, p._shared_posts_cursor)
                REMOVE p._followers_resume_hash, p._followees_resume_hash, p._posts_resume_hash
            """
